defmodule ExIRC.ChannelsTest do
  ...
  test "Can set the topic for a channel" do
    channels =
      Channels.init()
      |> Channels.join("#testchannel")
      |> Channels.set_topic("#testchannel", "Welcome to Test Channel!")
  ...
  test "Joining a users to a channel we aren't in is a noop" do
    ...
    assert {:error, :no_such_channel} ==
             Channels.channel_has_user?(channels, "#testchannel", "testnick")
  ...
  test "Renaming a user that doesn't exist is a noop" do
    channels =
      Channels.init()
      |> Channels.join("#testchannel")
      |> Channels.user_rename("testnick", "newnick")
  ...
  test "Can get the current set of channel data as a tuple of the channel name and it's data as a proplist" do
    ...
    testchannel =
      {"#testchannel", [users: ["testnick"], topic: "Welcome to Test!", type: :secret]}

    anotherchan =
      {"#anotherchan", [users: ["testnick"], topic: "Welcome to Another Channel!", type: :public]}
